# 내 PC에서 직접 크롤하기 (광고 전량 수집)

GitHub 자동 크롤은 서버 IP(데이터센터)라 메타가 브랜드당 **~30개만** 내줍니다.
내 PC(집 IP)에서 **화면을 띄워(headful)** 돌리면 봇 차단을 덜 받아 **훨씬 많이(보통 100개+)** 받아집니다.
신규 브랜드를 추가했을 때 이 방법으로 전량 수집하세요.

## 처음 한 번만 (설치)

1. 파이썬이 없다면 https://www.python.org/downloads/ 에서 설치
   (설치 화면에서 **"Add Python to PATH"** 꼭 체크)
2. 이 폴더의 **`setup-local.bat`** 더블클릭 → 자동으로 라이브러리·크롬엔진 설치
3. 같은 폴더의 **`.env.example`** 을 복사해 파일명을 **`.env`** 로 바꾼 뒤,
   메모장으로 열어 두 줄을 채웁니다:
   ```
   SUPABASE_URL=https://...supabase.co
   SUPABASE_SERVICE_KEY=eyJ... (service_role 키)
   ```
   - 값은 Supabase 대시보드 → **Project Settings → API** 에서 복사
   - ⚠️ `service_role` 키는 비밀키입니다. 누구에게도 공유/업로드하지 마세요.
     (`.env` 는 git 에 안 올라가게 되어 있습니다.)

## 매번 (크롤 실행)

웹에서 신규 브랜드를 추가한 뒤:

- **`crawl-new-1day.bat`** 더블클릭 → **오늘(최근 1일) 추가한 브랜드만** 전량 크롤 (방금 추가했으면 이거)
- **`crawl-new-7days.bat`** 더블클릭 → **최근 7일 내 추가한 브랜드만** 전량 크롤
- **`crawl-capped.bat`** 더블클릭 → **클라우드 30개 제한에 막혀 아직 다 못 가져온 브랜드만**(현재 광고 35개 이하) 골라 전량 보충. 이미 많이 쌓인 브랜드는 자동 제외 → 빠름.
- **`crawl-all.bat`** 더블클릭 → **등록된 모든 브랜드** 다시 크롤 (오래 걸림)

> 어떤 방식이든 **같은 Library ID(중복 소재)는 자동으로 건너뜁니다.** 이미 받은 광고는 다시 안 받고, 새 광고만 추가돼요. 그래서 여러 번 돌려도 안전합니다.

실행하면 크롬 창이 떠서 **스스로 스크롤**합니다. 끝날 때까지 창을 닫지 마세요.
다 끝나면 웹 갤러리에서 **새로고침** 하면 바로 반영됩니다.

## 다른 PC / 집에서 할 때 (IP가 바뀌어도 OK)
- **IP가 달라지는 건 전혀 문제 없습니다.** 오히려 집 IP(일반 가정용)가 회사 IP보다 메타 차단을 덜 받아 더 좋아요. IP 관련 별도 설정은 없습니다.
- **폴더를 그대로 들고 가도 됩니다.** 단, PC가 바뀌면 그 PC에서 **`setup-local.bat`을 한 번** 돌려야 해요(파이썬 가상환경 `.venv`와 크롬엔진은 PC마다 새로 깔려야 하므로). → crawl-*.bat 을 그냥 더블클릭해도, `.venv`가 없으면 **자동으로 설치부터** 진행합니다.
- **`.env`(키 파일) 챙기는 법**:
  - USB/드라이브로 **폴더째 복사**하면 `.env`도 같이 따라옵니다(그대로 사용 가능).
  - 만약 `git clone` 으로 받으면 `.env`는 안 따라옵니다(보안상 git 제외). 이 경우 `.env.example` 복사해서 키 두 줄을 다시 채우면 됩니다. 키 값은 어느 PC에서나 **동일**합니다(같은 Supabase 프로젝트).

## 참고
- 결과는 GitHub 크롤과 **같은 Supabase**에 저장됩니다(중복은 자동 누적, 새 광고만 추가).
- 영상 프레임 썸네일은 `ffmpeg` 가 있어야 생성됩니다(없어도 광고·영상은 정상 저장).
  필요하면 https://www.gyan.dev/ffmpeg/builds/ 에서 받아 PATH 에 추가하세요.
- 조절용 환경변수(고급): `CRAWL_SINCE_DAYS`(신규 기준 일수), `CRAWL_MAX_SCROLLS`(스크롤 횟수).
