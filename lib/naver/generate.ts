...
export interface DraftOpts {
  ...
  /* 완전 자동 발행용 '대충 쓴 일상글' 모드 — 사람 검수 없이 나가는 글이라 광고 오인 방지가 최우선.
     짧고 목적 없어 보이게: 소구점·강조어 반영을 끄고, 분량을 줄이고, 마무리도 흐리게. */
  casual?: boolean
}
...
export async function draftPost(
  ...
  const casual = opts.casual === true // 완전 자동 발행 글 — 소구점·강조어 무시, 더 짧고 무심하게
...
${casual ? `[🤖 자동 발행 모드 — '지나가던 회원이 대충 남긴 글' 톤. 광고로 오인되지 않는 게 최우선]
- 이번 글은 아무 목적 없는 평범한 커뮤니티 글이다. 홍보 뉘앙스·전문가 티·"알려드림" 톤 전부 금지.
- 짧게. 서론·정리 없이 한두 호흡에 툭 던지고 끝. 끝맺음은 대충 흐려도 좋다("아무튼 그렇네요", "...뭐 그런 날이네요", "다들 그런가요").
- 정보가 섞여도 한 조각만, 잘 쓰려고 다듬은 티를 내지 마라(단, 오탈자를 일부러 만들진 말 것 — 어색하다).
- ㅋㅋ/ㅠ 는 0~2개, 물음표 남발 금지. 특정 업체·서비스·브랜드·링크 언급 절대 금지.` : ''}
${!casual && cafe.selling_point ? `[이 채널이 글로 해내야 하는 것] ${cafe.selling_point}
  → 콘텐츠 목표(꿀팁·정보 나눔으로 팬층 쌓기 등)라면: 이 글이 실제로 그걸 수행해야 한다 — 진짜 써먹을 팁을 푼다.
  → 상업 목표(업체·서비스로 유도 등)라면: 본문에 직접 드러내지 마 — 업체명·홍보 티 금지, 결로만 배어나오게.` : ''}
${!casual && emphasis.length ? `[대화에 자연스럽게 스밀 단어(볼드 금지, 억지로 넣지 말 것)] ${emphasis.join(', ')}` : ''}
...
[본문]
- 위 유형을 컨셉의 말투로. ${casual ? '60~250자로 짧게(길어도 300자 — 대충 남긴 글은 원래 짧다)' : '100~450자(꿀팁방출·자료나눔은 700자까지 허용)'}.
