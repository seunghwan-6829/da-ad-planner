...
let googleMemCache: { targets: Target[]; ads: Ad[]; counts: Record<string, number>; complete: boolean } | null = null;
...
export default function GoogleAdsCrawlerPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    if (!bgLoadedRef.current) {
      ...
      if (hadCacheRef.current) {
        ...
        // 최초(캐시 없거나 부분): 전체 병렬 로드. 오류 없이 끝까지 받은 경우만 '완료'로 표시(→캐시 저장).
        (async () => {
          setSyncing(true);
          ...
          const worker = async () => {
            while (!done) {
              ...
              try {
                ...
                if (rows.length < PAGE) { done = true; break; } // 마지막 페이지(정상 종료)
  ...
  useEffect(() => {
    if (loading) return;
    googleMemCache = { targets, ads, counts, complete: loadComplete };
    if (!loadComplete) return; // 부분 데이터는 영구 캐시에 저장하지 않음(다음 진입 시 8천개만 뜨던 버그 방지)
    const t = setTimeout(() => { saveCache("google", ads, true); }, 1500);
