...
import { Folder, Search, Loader2, ChevronRight, Network, Trash2, Megaphone } from 'lucide-react'
import { Input } from '@/components/ui/input'
import { useAuth } from '@/lib/auth-context'
import { getClients, getClientsForUser, Client } from '@/lib/api/clients'
import { getMindmaps, deleteMindmap, Mindmap } from '@/lib/api/mindmaps'
              ...
              {mmLoading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {mindmaps.map((mm) => (
                    <div
                      key={mm.id}
                      onClick={() => router.push(`/plan-mindmap/${mm.id}`)}
                      className="group relative cursor-pointer overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-lg transition-shadow"
                    >
                      <div className="aspect-video w-full overflow-hidden bg-gray-100 dark:bg-gray-800">
                        {mm.source_thumb ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={mm.source_thumb} alt="" className="h-full w-full object-cover" loading="lazy" />
                        ) : (
                      ...
                      <div className="p-3">
                        <p className="line-clamp-2 text-sm font-semibold text-gray-800 dark:text-gray-100">{mm.title || '마인드맵'}</p>
                        <p className="mt-1 text-[11px] text-gray-400">{new Date(mm.created_at).toLocaleDateString('ko-KR')}</p>
                      </div>
                      <button
                        onClick={(e) => handleDelete(mm.id, e)}
                        className="absolute right-2 top-2 rounded-full bg-black/40 p-1.5 text-white opacity-0 transition-opacity hover:bg-red-500 group-hover:opacity-100"
                        title="삭제"
                      >
