...
// POST { target_id } → 광고주의 최근 광고 텍스트로 대분류 자동 판정(메타 categorize 미러) → ga_targets.category 갱신
export async function POST(req: Request) {
  ...
  const { data: target } = await supabaseAdmin.from('ga_targets').select('category').eq('id', targetId).single()
  const currentCategory = (target?.category || '').trim()

  const { data: ads } = await supabaseAdmin
    .from('ga_ads')
    .select('ad_text, page_name, landing_url')
    .eq('target_id', targetId)
    .order('first_seen_at', { ascending: false })
    .limit(8)

  if (!ads || ads.length === 0) return NextResponse.json({ category: null, reason: 'no_ads' })
  if (!apiKey) return NextResponse.json({ error: 'ANTHROPIC_API_KEY 없음' }, { status: 500 })

  const sample = ads
    .map((a, i) => `[${i + 1}] ${((a.ad_text ?? '') + ' ' + (a.landing_url ?? '')).replace(/\s+/g, ' ').slice(0, 300)}`)
    .join('\n')

  const prompt = `아래는 한 광고주의 최근 구글 광고(검색/유튜브/디스플레이) 카피·랜딩이야. 이 브랜드가 무엇을 파는지 파악해서:
...
${sample}`
  ...
  try {
    ...
    if (!res.ok) return NextResponse.json({ error: data.error?.message ?? 'AI 오류' }, { status: res.status })
    ...
    // 사용자가 이미 대분류를 지정한 경우(미분류 아님)는 보존
    const category = currentCategory && currentCategory !== '미분류' ? currentCategory : aiCategory

    await supabaseAdmin.from('ga_targets').update({ category, summary }).eq('id', targetId)
