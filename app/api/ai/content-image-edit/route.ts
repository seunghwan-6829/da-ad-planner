...
export async function POST(req: Request) {
  ...
  if (recreate) {
    // 첨부 이미지(원본 프레임)를 "거의 그대로 다시 촬영한 변형본"으로. 인물→제품 대체 같은 표류를 강하게 차단.
    const sanLine = sanitize ? '\n- 노출/속옷/선정적 요소는 건전하게 순화.' : ''
    // scene.prompt 전체를 그대로 주입하면(제품 언급 등) 표류하므로 "가벼운 수정 힌트"로만 취급.
    const userMod = prompt.trim() ? `\n[가벼운 수정 힌트(아래 사진을 벗어나지 않는 선에서만 참고)] ${prompt.trim()}` : ''
    editPrompt =
`[재현 모드 — 첨부한 "레퍼런스 사진"을 거의 그대로 다시 만들기]
첨부된 이 사진을 같은 장면으로 한 번 더 촬영한 듯한 "변형본"을 만들어라. 절대 새로운 장면을 상상해서 만들지 말 것. 첨부 사진이 사실상 정답이다.
- [반드시 동일] 피사체의 종류와 구성: 레퍼런스에 사람/신체가 있으면 결과에도 반드시 똑같은 구도의 사람/신체가 있어야 한다. 구도·카메라 앵글·포즈·시점·프레이밍·피사체의 위치와 크기 모두 동일.
- [살짝만 변형 허용] 옷의 재질·색상, 인물 외형의 사소한 디테일, 미세한 조명/배경 정도. (전체 인상은 동일하되 픽셀 단위 복제는 피함)
- [★ 절대 금지] 레퍼런스에 없는 새로운 사물·제품·용기(병/펌프통/패키지 등)·소품·배경을 만들어내는 것. 그리고 피사체(사람)를 제품 사진이나 정물/오브제로 바꾸는 것. 레퍼런스가 인물 사진이면 결과도 무조건 같은 인물 사진이어야 한다.
- [절대 금지] 화면의 글자·자막·캡션·로고·워터마크(레퍼런스에 있어도 전부 제거).${sanLine}${userMod}
— 광고/스톡 느낌이 아닌, 레퍼런스와 동일한 "실제 촬영 사진" 품질.`
  } else {
    editPrompt = sanitize
