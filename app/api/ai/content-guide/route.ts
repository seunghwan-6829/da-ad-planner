...
export async function POST(req: Request) {
  ...
  if (single) {
    const p1 = `너는 숏폼 광고 컨텐츠 디렉터다. 첨부된 "한 장면(스크린샷)"을 보고, 이 화면의 "비주얼(피사체/구도/배경/조명/분위기)"만 비슷하게 다시 만들기 위한 이미지 생성 프롬프트를 작성해라.
[★ 절대 규칙] 화면에 보이는 글자·자막·캡션·로고·워터마크·버튼/UI 는 전부 무시할 것. 프롬프트에 어떤 문구·텍스트·자막·숫자·타이포그래피도 넣지 마라(그대로 쓰면 생성 이미지에 글자가 박혀서 일일이 지워야 한다). "글자가 단 하나도 없는 깨끗한 비주얼"을 묘사해라.
[참고] 브랜드: ${ad.page_name ?? '미상'}
JSON 으로만 작성:
- prompt: 이 화면을 만들 한국어 이미지 생성 프롬프트(피사체/구도/조명/분위기/스타일/카메라 앵글, 2~4문장 — 텍스트/자막은 언급조차 금지)
...
각 장면마다, 그 화면의 "비주얼(피사체/구도/배경/조명/분위기)"만 비슷하게 다시 만들기 위한 이미지 생성 프롬프트를 작성해라.

[★ 절대 규칙] 화면에 보이는 글자·자막·캡션·로고·워터마크·버튼/UI 는 전부 무시할 것. 프롬프트에 어떤 문구·텍스트·자막·숫자·타이포그래피도 넣지 마라(그대로 쓰면 생성 이미지에 글자가 박혀서 일일이 지워야 한다). "글자가 단 하나도 없는 깨끗한 비주얼"을 묘사해라.
[참고] 브랜드: ${ad.page_name ?? '미상'}

각 장면(scene)마다 다음을 작성(첨부된 이미지 순서와 1:1로 매칭, ${images.length}개):
- prompt: 그 화면을 만들기 위한 "이미지 생성 프롬프트"(한국어, 구체적 — 피사체/구도/조명/분위기/스타일/카메라 앵글 등 포함, 2~4문장. 텍스트/자막은 언급조차 금지)
- description: 이 장면이 무엇인지 간략 설명(20자 내외)
- caution: 이 장면을 만들 때 주의할 점(25자 내외)
